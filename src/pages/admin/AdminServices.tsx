...
const emptyForm = { title: "", slug: "", description: "", icon_name: "Globe", items: "", detail_content: "", display_order: 0, is_published: true };
...
const AdminServices = () => {
  ...
  const openEdit = (s: Service) => {
    setForm({ title: s.title, slug: s.slug, description: s.description ?? "", icon_name: s.icon_name ?? "", items: s.items?.join(", ") ?? "", detail_content: s.detail_content ?? "", display_order: s.display_order, is_published: s.is_published });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const payload = { title: form.title, slug: form.slug, description: form.description || null, icon_name: form.icon_name || null, items: form.items ? form.items.split(",").map(i => i.trim()).filter(Boolean) : null, detail_content: form.detail_content || null, display_order: form.display_order, is_published: form.is_published };
    ...
    <div>
          ...
          {services.map((s) => (
            <div key={s.id} className="flex items-center gap-4 p-4 rounded-xl bg-card border border-border">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">{s.slug} · {s.items?.length ?? 0} items</p>
              </div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><Label>Slug</Label><Input value={form.slug} onChange={(e) => setField("slug", e.target.value)} placeholder="web-development" /></div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Icon (Lucide name)</Label><Input value={form.icon_name} onChange={(e) => setField("icon_name", e.target.value)} placeholder="Globe" /></div>
            ...
            <div><Label>Description</Label><Textarea value={form.description} onChange={(e) => setField("description", e.target.value)} rows={2} /></div>
            <div><Label>Items (comma-separated)</Label><Input value={form.items} onChange={(e) => setField("items", e.target.value)} placeholder="Company Profile, Landing Page, E-Commerce" /></div>
            <div><Label>Detail Content (Markdown)</Label><Textarea value={form.detail_content} onChange={(e) => setField("detail_content", e.target.value)} rows={6} /></div>
      ...
      <Dialog open={priceOpen} onOpenChange={setPriceOpen}>
        <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Pricelist — {services.find(s => s.id === priceServiceId)?.title}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            {prices.map((p) => (
      ...
      <Dialog open={priceFormOpen} onOpenChange={setPriceFormOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div><Label>Nama Paket</Label><Input value={priceForm.name} onChange={(e) => setPField("name", e.target.value)} placeholder="Basic" /></div>
            ...
            <div><Label>Description</Label><Textarea value={priceForm.description} onChange={(e) => setPField("description", e.target.value)} rows={2} /></div>
            <div><Label>Features (comma-separated)</Label><Input value={priceForm.features} onChange={(e) => setPField("features", e.target.value)} placeholder="5 Halaman, Responsive, SEO Basic" /></div>
            <div className="grid grid-cols-2 gap-4">
