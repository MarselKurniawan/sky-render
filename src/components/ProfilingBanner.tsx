...
const ProfilingBanner = () => {
  ...
  return (
    <ScrollReveal className="lg:w-[380px] shrink-0" variant="fade">
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-navy via-navy-light to-electric/30 p-7 h-full min-h-[400px] flex flex-col justify-between transition-all duration-500">
        {/* Glow effects */}
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-5">
            <div className="w-11 h-11 rounded-xl bg-electric/15 backdrop-blur flex items-center justify-center border border-electric/20">
              <Icon size={22} className="text-electric-light" />
            </div>
            <span className="text-[10px] font-bold uppercase tracking-widest text-electric-light bg-electric/10 px-3 py-1 rounded-full">
              {current.tag}
            </span>
          </div>
          ...
          <p className="text-primary-foreground/70 text-sm leading-relaxed mb-5">{current.description}</p>
          ...
          <a
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-electric px-5 py-2.5 text-sm font-bold text-accent-foreground hover:bg-electric-light transition-all hover:shadow-[0_0_25px_hsl(217_91%_60%/0.35)] hover:scale-105 active:scale-100"
          >
          ...
          <div className="flex gap-1.5">
            {slides.map((_, i) => (
              <button
                key={i}
                onClick={() => setActive(i)}
                className={`h-1.5 rounded-full transition-all ${i === active ? "w-6 bg-electric-light" : "w-1.5 bg-primary-foreground/30"}`}
              />
            ))}
          </div>
          <div className="flex gap-2">
            <button onClick={prev} className="w-8 h-8 rounded-full bg-electric/15 backdrop-blur flex items-center justify-center text-primary-foreground hover:bg-electric/25 transition-colors border border-electric/10">
              <ChevronLeft size={16} />
            </button>
            <button onClick={next} className="w-8 h-8 rounded-full bg-electric/15 backdrop-blur flex items-center justify-center text-primary-foreground hover:bg-electric/25 transition-colors border border-electric/10">
